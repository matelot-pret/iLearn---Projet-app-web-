...
$cours = $db->get_cours_with_ressources(); // N'OUBLIE PAS D'AJOUTER LA MÉTHODE MEC SINON BOOM💥
                ...
                <div class="card bg-<?php echo $couleur; ?> bg-opacity-25 border-0 rounded-4 h-100">
                    <a href="lesson.php?id=<?= $c['id']?>" class="text-decoration-none text-black">
                        <div class="card-body p-4 d-flex flex-column justify-content-between" style="min-height: 300px;">
                            <h3 class="card-title fw-bold fs-2"><?php htmlspecialchars($c['nom']) ?></h3>
                            <div>
                                <p class="card-text mb-1 fs-5">Bloc <?php htmlspecialchars($c['bloc']) ?></p>
                                <p class="card-text mb-1 fs-5">Section <?php htmlspecialchars($c['section']) ?>
